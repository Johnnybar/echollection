...
class LinksScreen extends React.Component {
	...
	render() {
		return (<View style={styles.container}>
			{/* In case of error */}
			{
				this.state.errorWhenMounting && (<View>
			...
			{/* If not loaded yet */}
			{
				this.state.mounted === false && (<Animated.View style={{
					...styles.contentContainer,
					top: 200,
					left: this.state.startScreenLogoLeftPosition,
					opacity: this.state.startScreenLogoOpacity
				}}>
					<Button title="Welcome!" textStyle={styles.playButtonText} titleStyle={{
						fontWeight: "700"
					}} buttonStyle={{
			...
			{/* After game loaded */}
			{
				this.state.mounted === true && (<ScrollView style={styles.container} contentContainerStyle={styles.contentContainer}>
					{/* After winning the game */}
					{
						this.state.gameWon === true && (<View>
							<Text style={styles.success}>YOU WON THE GAME, CONGRATS!</Text>
							<Button title="Start Again" onPress={() => {
								this._restartGame()
							}}/>
						</View>)
					}
					...
					{/* When still playing */}
					{
						this.state.gameWon !== true && (<View style={styles.container}>
							...
							{
								this.state.success === true && this.props.level < 12 && (<Text style={styles.success}>
									GREAT JOB, HERE COMES THE NEXT ONE
								</Text>)
							}
							<Button title="next" textStyle={{
								fontSize: 10,
								fontWeight: "400"
							}} buttonStyle={{
								height: 23,
							...
							<View style={styles.buttonsContainer}>
								...
								<TouchableOpacity style={styles.instrumentContainer} onPress={() => {
									...
									<CustomIcon name="cowbell" size={80 - 2} title="COWBELL" raised rounded style={{
							...
							<Animated.View style={{
								...
								<Button title="Play" rounded onPress={() => {
									...
									styles.playButtonText, this.state.currentlyPlaying === true
										? styles.playButtonOff
										: styles.playButtonOn
								]} buttonStyle={[
									styles.playButtonBg, this.state.currentlyPlaying === true
										? styles.playButtonOff
										: styles.playButtonOn
								]}/>
							</Animated.View>
							<View style={styles.buttonsContainer}>
								<TouchableOpacity style={styles.instrumentContainer} onPress={() => {
									...
									<CustomIcon name="hat" size={75 + 6} title="HAT" raised rounded style={{
								...
								{
									this.props.level > 8 && (<TouchableOpacity style={styles.instrumentContainer} onPress={() => {
										this._playerInput("jingle")
									}}>
										<CustomIcon name="jingle" size={65 - 6} title="JINGLE" raised rounded style={{
...
const mapStateToProps = state => {
	return {level: state.level, speed: state.speed, muted: state.muted, restart: state.restart}
}
...
const progressIndicatorMarginBottom = Platform.OS === "ios"
	? 60
	: 10
